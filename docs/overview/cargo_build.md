...
Crubit provides an automated helper script `cargo/build/setup_bazel_env.py` that
queries and builds all required C++ libraries using Bazel, merges them into
monolithic static archives, pre-generates Protobuf headers, and exports the
necessary environment variables.

1. **Set up the Bazel environment and prebuild C++ dependencies**:
