...
def generate_protobuf_headers(protoc_bin, out_dir, repo_root=REPO_ROOT):
    ...
    existing_protos = [
        os.path.join(repo_root, p) for p in PROTO_FILES
        if os.path.exists(os.path.join(repo_root, p))
    ]
    if not existing_protos:
        print(f"Warning: No Crubit .proto files found under {repo_root}", file=sys.stderr)
        return
...
def main():
    ...
    generate_protobuf_headers(args.protoc, os.path.abspath(args.out_dir), os.path.abspath(args.repo_root))
